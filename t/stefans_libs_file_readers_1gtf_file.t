...
$exp = [ 'seqname', 'source', 'feature', 'start', 'end', 'score', 'strand', 'frame', 'gene_id', 'gene_type', 'gene_status', 'gene_name', 'level', 'havana_gene', 'transcript_id', 'transcript_type', 'transcript_status', 'transcript_name', 'tag', 'transcript_support_level', 'havana_transcript', 'exon_number', 'exon_id' ];
...
ok( $test_object->{'__max_header__'} == 23, "internal Max_Header \$test_object ($test_object->{'__max_header__'})" );
...
is_deeply( $test_object->get_line_asHash(2), $before, "The select_where run did not modify the table structure" );
...
ok( $value->{'__max_header__'} == 23, "internal Max_Header subset ($value->{'__max_header__'})" );

#print "The pdl should just not throw any errors...\n";
@values = $value->get_pdls_4_chr('chr1')->unpdl();
