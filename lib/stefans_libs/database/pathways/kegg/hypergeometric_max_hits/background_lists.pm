package stefans_libs::database::pathways::kegg::hypergeometric_max_hits::background_lists;
...
sub new {
    ...
    $self = {
        debug => $debug,
        dbh   => $dbh,
        '_tableName' => "hypergeometric_background_lists",
    };

    bless $self, $class if ( $class eq "stefans_libs::database::pathways::kegg::hypergeometric_max_hits::background_lists" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "hypergeometric_background_lists";
     push ( @{$hash->{'variables'}},  {
     ...
     $self->{'UNIQUE_KEY'} = [ 'reference_dataset' ];
