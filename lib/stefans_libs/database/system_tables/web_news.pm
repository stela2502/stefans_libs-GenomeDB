...
sub new {
    ...
    bless $self, $class if ( $class eq "web_news" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'title', 'date' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'title', 'date' ];
...
sub get_news_hash{
	...
	my $data = $self->getArray_of_Array_for_search({
 	'search_columns' => ["title",'date','info'],
	'order_by' => [[ 'my_value', '-', 'date'], ['my_value', '-', 'id']]
	});
	## Carp::confess ( "just for debug: $self->{'complex_search'}\n");
 	my ($return, $processed, $array);
 	$return = [];
 	foreach my $info_array ( @$data ){
 		unless ( defined $processed -> { @$info_array[1] }){
 			$processed -> { @$info_array[1] }  = 1;
 			push ( @$return, { 'date' =>  @$info_array[1], 'notes' => [] } );
 		}
 		push ( @{@$return[@$return -1] ->{'notes'}}, { 'title' => @$info_array[0], 'text' => @$info_array[2]})
 	}
 	return $return;
}
